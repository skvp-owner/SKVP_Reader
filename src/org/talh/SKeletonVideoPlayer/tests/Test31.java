...
import org.talh.SKeletonVideoPlayer.Defs;
...
public class Test31 extends AbstractTest {

	protected int run() {
		ClassLoader cl = this.getClass().getClassLoader();
		URL resourceUrl = cl.getResource("org/talh/SKeletonVideoPlayer/tests/resources/file29.skvp");
		if (resourceUrl == null) {
			return 1;
		}
		...
		try {
			if (reader.getJointRadiuses()[1] != Defs.DEFAULT_RADIUS) {
				return 37;
			}
		} catch (SKVPNonInitializedReaderException e) {
			return 38;
		...
		try {
			if (reader.getConnectionsRadius() != Defs.DEFAULT_CONNECTION_RADIUS) {
				return 39;
			}
		} catch (SKVPNonInitializedReaderException e) {
			return 40;
		...
		try {
			if (reader.getJointColors()[1].getDiffuseColor() != Defs.DEFAULT_DIFFUSE_COLOR) {
				return 41;
			}
			if (reader.getJointColors()[0].getSpecularColor() != Defs.DEFAULT_SPECULAR_COLOR) {
				return 42;
			}
		} catch (SKVPNonInitializedReaderException e) {
			return 43;
		...
		try {
			if (reader.getConnectionsColor().getDiffuseColor() != Defs.DEFAULT_CONNECTION_DIFFUSE_COLOR) {
				return 44;
			}
			if (reader.getConnectionsColor().getSpecularColor() != Defs.DEFAULT_CONNECTION_SPECULAR_COLOR) {
				return 45;
			}
		} catch (SKVPNonInitializedReaderException e) {
			return 46;
		...
		try {
			if (reader.getCameraLocation().getX() != 0) {
				return 47;
			}
			if (reader.getCameraLocation().getY() != 0) {
				return 48;
			}
			if (reader.getCameraLocation().getZ() != 0) {
				return 49;
			}
		} catch (SKVPNonInitializedReaderException e) {
			return 50;
